...
def train_gpt(metadatas, num_epochs, batch_size, grad_acumm, output_path, max_audio_length, max_text_length, lr, weight_decay, save_step, run_name):
    ...
    config.num_loader_workers = 1  # Use 1 worker - safe for DDP, faster than 0
    config.pin_memory = True  # Speed up CPU->GPU transfer
    config.persistent_workers = True  # Avoid worker respawn overhead between epochs
